 ...
 * related hazards. This <b>extends</b> {@link CommonGridView}.
 * 
...
public class CausalFactorsView extends CommonGridView<ICausalFactorDataModel> {
  ...
  public void createPartControl(Composite parent) {
    ICausalFactorDataModel dataModel = (ICausalFactorDataModel) ProjectManager
        .getContainerInstance().getDataModel(this.getProjectID());
  ...
  protected void fillTable() {
    // The column labels are set depending on whether the user wants to use Scenarios, than an extra
    // column for scenarios is added or not
    ...
    for (IRectangleComponent component : components) {
      if (!isCFFiltered(component)) {
        ...
        // here all child rows containing the causal factors for the current component are created
        if (setting.addPrimaryFactor(component, componentRow)) {
  ...
  private void createScenarioRow(Link factorComponentLink, GridRow entryRow) {
    ...
    scenarioCell.addCellButton(new CellButtonLinking<ContentProviderScenarios>(getGridWrapper(),
        new ContentProviderScenarios(getDataModel(), factorComponentLink, uca),
   ...
   * @return
   */
  private GridRow createHazardRow(GridRow entryRow, Link entryHazLink) {
    ITableModel hazard = getDataModel().getHazard(entryHazLink.getLinkB());
    String hazText = hazard != null ? hazard.getIdString() + " - " + hazard.getTitle()
        : "no hazard";
    ...
    CellEditorSafetyConstraint cell = new CellEditorSafetyConstraint(getGridWrapper(),
        getDataModel(), entryHazLink);
    Link factorComponentLink = getDataModel().getLinkController()
        .getLinkObjectFor(LinkingType.UcaCfLink_Component_LINK, entryHazLink.getLinkA());
    Link ucaCfLink = getDataModel().getLinkController()
        .getLinkObjectFor(LinkingType.UCA_CausalFactor_LINK, factorComponentLink.getLinkA());
    ...
    Optional<Link> safetyOption = getDataModel().getLinkController()
        .getRawLinksFor(LinkingType.CausalHazLink_SC2_LINK, entryHazLink.getId()).stream()
        .findFirst();
    ...
    entryRow.addCell(6, new CellEditorFactorNote(getGridWrapper(), getDataModel(), entryHazLink));
    return null;
  ...
  private GridRow createSingleConstraintRow(GridRow entryRow, Link factorConstraintLink) {
    Link factorComponentLink = getDataModel().getLinkController()
        .getLinkObjectFor(LinkingType.UcaCfLink_Component_LINK, factorConstraintLink.getLinkA());
    ...
    CellEditorFactorNote hintCell = new CellEditorFactorNote(getGridWrapper(), getDataModel(),
        factorConstraintLink);
  ...
  private void createScenarioRow(GridRow entryRow, Link scenarioLink) {
    Link factorComponentLink = getDataModel().getLinkController()
        .getLinkObjectFor(LinkingType.UcaCfLink_Component_LINK, scenarioLink.getLinkA());
    addHazardCell(entryRow, factorComponentLink);
  ...
  /**
   * This setting object decides whether the causal Factors are displayed like:<br> <ul> <li>1 CF
   * contains n UCA, or <li>1 UCA contains n CF </ul>
   * 
   */
  ...
  private class LegacySetting implements CausalMethodologySetting {
    ...
    /**
     * This implementation of the method creates a
     */
    @Override
    public boolean addPrimaryFactor(IRectangleComponent component, GridRow componentRow) {
      ...
      for (ITableModel factor : ucaCfLink_Component_To_CF_map.keySet()) {
        if (factor != null && !isFiltered(factor.getText(), CAUSALFACTORS)) {
          ...
          causalFactorRow = new GridRow(getGridWrapper().getColumnLabels().length, 1,
              new int[] { 1 });
          ...
          for (Link factorComponentLink : factorComponentLinkList) {
             ...
             * the hazards are either based on the uca or linkable
             */
            Link ucaCFLink = getDataModel().getLinkController().getLinkObjectFor(
                LinkingType.UCA_CausalFactor_LINK, factorComponentLink.getLinkA());
            IUnsafeControlAction uca = getDataModel().getControlActionController()
                .getUnsafeControlAction(ucaCFLink.getLinkA());
            ucaList.add(uca);
     ...
     */
    public void addSecondaryFactor(GridRow factorRow, Link factorComponentLink,
      ...
      for (Link link : getDataModel().getLinkController().getRawLinksFor(entryType,
        ...
        case CausalEntryLink_Scenario_LINK: {
          if (getDataModel().getExtendedDataController()
              .getRefinedScenario(link.getLinkB()) == null) {
            ProjectManager.getLOGGER()
                .error("Causal Factor Scenario link with illegal Scenario id!");
          } else {
  ...
  private class UCAScenarioSetting implements CausalMethodologySetting {
    ...
    public boolean addPrimaryFactor(IRectangleComponent component, GridRow componentRow) {
      ...
      Map<IUnsafeControlAction, List<Link>> ucaCfLink_Component_To_UCA_map = getDataModel()
          .getCausalFactorController().getUCABasedMap(component, getDataModel().getLinkController(),
              getDataModel().getControlActionController());
      List<IUnsafeControlAction> ucaList = new ArrayList<>();
      // iterates all UCAs stored in the data model and included in the
      // ucaCfLink_Component_To_UCA_map as keys to a list of links to causal factors
      for (IUnsafeControlAction uca : ucaCfLink_Component_To_UCA_map.keySet()) {
        if (uca != null && !isFiltered(uca.getId(), "UCA")) {
          // addComponentRow is only set true if the uca is not filtered so it wont appear in the
          // grid
          ...
          if (checkAccess(AccessRights.WRITE)) {
            addEntriesRow.addCell(2,
                new GridCellButtonAddCausalFactor(component, getDataModel(), uca.getId()));
          }
      ...
      if (!getActiveCategory().equals("UCA")) {
        GridRow buttonRow = new GridRow(getGridWrapper().getColumnLabels().length);
        buttonRow.addCell(1,
            new GridCellButtonAddUCAEntry(component, null, getDataModel(), getGrid(), ucaList));
        buttonRow.setColumnSpan(1, getGridWrapper().getColumnLabels().length - 2);
        componentRow.addChildRow(buttonRow);
        addComponentRow = true;
      }
    ...
    public void addSecondaryFactor(GridRow factorRow, Link factorComponentLink, Link ucaCFLink,
        ITableModel factor) {
      // if the link between uca and causal Factor contains a non existent uuid than it should be
      // deleted,
      // however if the value is null than the link is treated as the initial link that links the
      // uca to the
      ...
      GridRow entryRow = new GridRow(getGridWrapper().getColumnLabels().length, 1, new int[] { 2 });
      CellEditorCausalFactor cell = new CellEditorCausalFactor(getGridWrapper(), getDataModel(),
          factor.getId(), factorComponentLink);
