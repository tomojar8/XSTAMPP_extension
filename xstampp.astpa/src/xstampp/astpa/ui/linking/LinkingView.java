...
package xstampp.astpa.ui.linking;
...
public class LinkingView extends StandartEditorPart implements IPropertyChangeListener {
   ...
   */
  public static final String ID = "astpa.steps.step1_4";
  ...
  /**
   * Returns the selected item in the available table. Returns null if nothing is selected.
   * 
  ...
  private Object[] getSelectedAvailable() {
    if (!LinkingView.this.availableTableViewer.getSelection().isEmpty()) {
      return ((IStructuredSelection) LinkingView.this.availableTableViewer.getSelection())
          .toArray();
    }
  ...
  /**
   * Returns the selected item in the current table. Returns null if nothing is selected.
   * 
   * @return the selected item
   * 
   * @author Patrick Wickenhaeuser
  ...
  /**
   * Removes all accidents/hazards, currently linked to the selected accidents/hazards.
   * 
   * @author Patrick Wickenhaeuser
   */
  ...
  /**
   * Removes the currently selected hazards/accidents in the current table, if any, from the linked
  ...
  public void createPartControl(Composite parent) {
    ...
    this.selectionTableViewer = new TableViewer(tableLayoutComposite,
    ...
    TableViewerColumn selectionTableIdColumn = new TableViewerColumn(this.selectionTableViewer,
        SWT.NONE);
    ...
    TableViewerColumn selectionTableTitleColumn = new TableViewerColumn(this.selectionTableViewer,
        SWT.NONE);
    ...
    this.availableTableViewer = new TableViewer(availableTableLayoutComposite,
    ...
    TableViewerColumn availableTableIdColumn = new TableViewerColumn(this.availableTableViewer,
        SWT.NONE);
    availableTableIdColumn.setLabelProvider(idLabelProvider);

    TableViewerColumn availableTableTitleColumn = new TableViewerColumn(this.availableTableViewer,
        SWT.NONE);
    ...
    this.currentTableViewer = new TableViewer(currentTableLayoutComposite,
        SWT.FULL_SELECTION | SWT.MULTI | SWT.V_SCROLL);
    ...
    TableViewerColumn currentViewerColumnId = new TableViewerColumn(this.currentTableViewer,
        SWT.NONE);
    ...
    TableViewerColumn currentViewerColumnShortDescription = new TableViewerColumn(
        this.currentTableViewer, SWT.NONE);
    ...
    tableLayout.setColumnData(selectionIdColumn, new ColumnWeightData(
        LinkingView.COLUMN_ID_WEIGHT_X, LinkingView.COLUMN_ID_WEIGHT1_Y, false));
    ...
    tableLayout.setColumnData(currentColumnTitle, new ColumnWeightData(
        LinkingView.COLUMN_TITLE_WEIGHT_X, LinkingView.COLUMN_TITLE_WEIGHT_Y, false));
    tableLayout.setColumnData(availableColumnId, new ColumnWeightData(
        LinkingView.COLUMN_ID_WEIGHT_X, LinkingView.COLUMN_ID_WEIGHT1_Y, false));
    tableLayout.setColumnData(availableColumnTitle, new ColumnWeightData(
        LinkingView.COLUMN_TITLE_WEIGHT_X, LinkingView.COLUMN_TITLE_WEIGHT_Y, false));
  ...
  public void propertyChange(PropertyChangeEvent event) {
    LinkingView.this.lblSelected.setFont(new Font(Display.getCurrent(),
        PreferenceConverter.getFontData(IEditorBase.STORE, IPreferenceConstants.DEFAULT_FONT)));
    LinkingView.this.lblAvailableForLinking.setFont(new Font(Display.getCurrent(),
        PreferenceConverter.getFontData(IEditorBase.STORE, IPreferenceConstants.DEFAULT_FONT)));

    LinkingView.this.lblCurrentlyLinked.setFont(new Font(Display.getCurrent(),
        PreferenceConverter.getFontData(IEditorBase.STORE, IPreferenceConstants.DEFAULT_FONT)));
  }
}
