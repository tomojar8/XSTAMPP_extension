 ...
 * @since 2.0
 */
public class SystemDescriptionView extends StandartEditorPart
    implements ITextEditor, IPropertyChangeListener {
  ...
  public void createPartControl(Composite parent) {
    ...
    final int blueB = 255;
    this.textBackgroundColor = new Color(Display.getDefault(), new RGB(redB, greenB, blueB));

    // Initialize with color white
    final int redW = 0;
    final int greenW = 0;
    final int blueW = 0;
    this.textForegroundColor = new Color(Display.getDefault(), new RGB(redW, greenW, blueW));
      ...
      public void widgetSelected(SelectionEvent e) {
        for (ISelectionChangedListener obj : SystemDescriptionView.this.listener) {
          obj.selectionChanged(
              new SelectionChangedEvent(SystemDescriptionView.this, getSelection()));
  ...
  /**
   * Stores the current description text and style ranges of the text in the data model. Style
   * ranges are used to set the style and format of the description text.
   * 
  ...
  /**
   * Gets the current project description from the data model and set the text formation.
   * 
   * @author Patrick Wickenhaeuser, Sebastian Sieber
   */
  ...
  private void createContextMenu() {
    ...
    menu.addMenuListener(new MenuAdapter() {
      ...
      public void menuShown(MenuEvent event) {
        int selectionCount = SystemDescriptionView.this.descriptionText.getSelectionCount();
        cutItem.setEnabled(selectionCount > 0);
        copyItem.setEnabled(selectionCount > 0);
        selectAllItem
            .setEnabled(selectionCount < SystemDescriptionView.this.descriptionText.getCharCount());
      }
    });
  ...
  /**
   * Force the focus on project name and select text if text equals "New Project"
   * 
   * @author Sebastian Sieber
   * 
   * @param composite
      ...
      composite.getDisplay().asyncExec(new Runnable() {
        ...
        public void run() {
          ...
          SystemDescriptionView.this.projectNameText.setSelection(0,
              SystemDescriptionView.this.projectNameText.getText().length());
        }
      });
    }
  }
  ...
  private void addStyleRangesFor(String style, String fontName, int fontSize, int fontStyle) {
    ...
    for (int i = selectionRange.x; i < selectionRange.x + selectionRange.y; i++) {
      /*
       * if there is a range that starts before or at the selected region it is than cloned and
       * adapted if the range that was found has a length greater than 0 the index is increased
       */
      if (ranges.length > next && ranges[next].start <= i) {
        if (newRange != null) {
          descriptionText
              .setStyleRange(setFontItemRange(style, shouldSET, newRange, fontSize, fontName));
          newRange = null;
        }
         ...
         * which is calculated from the old length minus the offset between the index and the range
         * start
         */
        ...
        if (range.length > 0) {
          ...
          descriptionText
              .setStyleRange(setFontItemRange(style, shouldSET, range, fontSize, fontName));
        }
        next++;
        ...
        newRange.font = FontDescriptor.createFrom(TextToolbarContribution.SYSTEM_FONT)
            .createFont(null);
        newRange.start = i;
        newRange.length = 1;
      }
  ...
  /**
   * Set the style range if text get modified and widget is selected. Also triggers if text get
   * selection and widget is selected.
   * 
   * @author Sebastian Sieber
   * 
   * @param style
   ...
   */
  private StyleRange setFontItemRange(String style, boolean setProp, StyleRange styleRange,
  ...
  /**
   * Handles changes occurring in the TextArea depending on the CaretOffset and selection of the
   * text.
   * 
   * @author Sebastian Sieber
   * @param event
  ...
  public void updateStatusBar() {
    ...
    this.statusBar.setText(Messages.BackSlashSpaceOffset + offset + " " + Messages.Line //$NON-NLS-1$
        + lineIndex + "\t"); //$NON-NLS-1$
  }
  ...
  public void propertyChange(PropertyChangeEvent event) {
    ...
    this.projectNameLabel.setFont(new Font(Display.getCurrent(),
        PreferenceConverter.getFontData(IEditorBase.STORE, IPreferenceConstants.DEFAULT_FONT)));
