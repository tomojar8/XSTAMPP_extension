		...
		<fo:table border="none" space-after="30pt">
			...
			<fo:table-header border="none" background-color="#1A277A"
				...
				<fo:table-row>
					...
					<fo:table-cell padding="3px">
						<fo:block font-weight="bold">Wrong timing causes hazard </fo:block>
					</fo:table-cell>
				</fo:table-row>
			</fo:table-header>
						...
						<xsl:for-each select="cac/controlactions/controlaction">
							...
							<xsl:choose>
								<xsl:when test="rules/rule">
									<fo:table-row border-bottom="solid" border-top="solid"
	...
	<xsl:template name="provided">
		<fo:table>
			...
			<fo:table-body>
				...
				<xsl:for-each select="rules/rule[type='provided']">
					<fo:table-row>
						<!-- Sets the row-colour -->
						<xsl:if test="position() mod 2 = 0">
