         ...
         <fo:page-sequence white-space-collapse="true" id="total">  
            ...
            <fo:flow flow-name="xsl-region-body">
				...
				<fo:block>
					...
					<xsl:for-each select="cac/controlactions/controlaction[isSecurityCritical = 'true']">
						<xsl:choose>
								<!-- ################### Context Table Template ################### -->
						...
						<xsl:choose>
							<xsl:when test="dependenciesForNotProvided/variableName and PMCombisWhenNotProvided/combinationOfPMValues[valueNames/name]">
								<fo:block space-after="5pt" page-break-after="avoid">
									...
									<xsl:attribute name="font-size"><xsl:value-of select="$title.size" />pt</xsl:attribute>
									   Context Table of control action <xsl:value-of select="title" /> in context not provided
								</fo:block> 
								<fo:block  text-align="center" page-break-after="always">
			...
			<fo:table-header border="solid" background-color="#1A277A"
				...
				<!-- Sets the PDF-Theme-Color -->
					...
					<fo:table-row keep-together.within-column="always" >
						...
						<xsl:for-each select="dependenciesForProvided/variableName">
							<fo:table-cell padding="3px" number-rows-spanned="2">
								<fo:block wrap-option="wrap" font-weight="bold"><xsl:value-of select="." /></fo:block>
							</fo:table-cell>
						</xsl:for-each>
						
						<fo:table-cell padding="3px" number-columns-spanned="3">
							<fo:block text-align="center" font-weight="bold" border-style="none">Vulnerable control action if provided</fo:block>
						</fo:table-cell>
					</fo:table-row>
					<fo:table-row>
			...
			<fo:table-header  border="solid" background-color="#1A277A"
				...
				<!-- Sets the PDF-Theme-Color -->
					...
					<fo:table-row keep-together.within-column="always" >
						...
						<fo:table-cell padding="3px">
							<fo:block font-weight="bold">Vulnerable if not provided in this context</fo:block>
						</fo:table-cell>
					</fo:table-row>
				</fo:table-header>
				<fo:table-footer>
