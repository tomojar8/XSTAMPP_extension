...
public class RefinedSafetyEntry implements Comparable<RefinedSafetyEntry> {
  ...
  /**
   * This private constructor is only called by one of the four static methods provided by this
   * class
   * 
   * @param number
   *          the number of the created Refined Data must be a positive nr > 0
   * @param type
   *          the Type of the context the rule should be generated for one of the <code>TYPE</code>
   *          constants
   *          Defined in IValueCombie
   * @param context
   *          the context of the refinement
   * @param var
   *          the variable object which represents a hazardous value combination
   * @param controller
   *          the astpa data model in which the refined data should be stored
   * 
   * @see IValueCombie
   * @see RefinedSafetyEntry#getTooLateEntry(int, ProcessModelVariables, IExtendedDataModel)
   * @see RefinedSafetyEntry#getTooEarlyEntry(int, ProcessModelVariables, IExtendedDataModel)
   ...
   * @see RefinedSafetyEntry#getNotProvidedEntry(int, ProcessModelVariables, IExtendedDataModel)
   */
  private RefinedSafetyEntry(int number, String type, String context, ContextTableCombination var,
      IExtendedDataModel controller) {
   ...
   * 
   * @param equalsSeq
   *          the sequence that is used to separate the variable-value tuples
   * @param andSeq
   *          the sequence that is used to separate the variable string from the value string
   *          <p>
   *          NOTE: all Whitespaces are ignored
   * @param useBrackets
   *          whether or not the tuples should put into brackets
   * @param parseBoolean
   *          whether or not getPmValues should translate any boolean expressions into natural
   *          language
   * @param useSpaces
   *          TODO
  ...
  public String getUCALinks() {
    ...
    for (UUID ucaID : variable.getUcaLinks(type)) {
      tempUcas = tempUcas.concat(model.getControlActionController().getUnsafeControlAction(ucaID).getIdString() + ",");
    }
  ...
  private void update() {
    ...
    if (this.getDataRef() != null) {
      model.getExtendedDataController().updateRefinedRule(getDataRef(), data, variable.getLinkedControlActionID());
    } else {
      setDataRef(model.getExtendedDataController().addRuleEntry(IExtendedDataModel.ScenarioType.BASIC_SCENARIO, data,
          variable.getLinkedControlActionID(),
          type, model.getLinkController()));
    }
  ...
  public String getRelatedHazards() {
    ...
    for (UUID ucaID : variable.getUcaLinks(type)) {
      for (ITableModel tableModel : model.getLinkedHazardsOfUCA(ucaID)) {
        tempHazLinks = tempHazLinks.concat(tableModel.getIdString() + ",");
      }
    }
  ...
  private void calcLTL() {
    ...
    final String CONTROLACTION = "(controlAction" + EQUALS
        + getCombination().getLinkedControlActionName().replaceAll(" ", "") + ")";
   ...
   * 
   * @param p
   *          the formular which sahould be valid in the next step
  ...
  public String getConstraintID() {
    return constraintPrefix + this.number;
  }

  public String getRefinedUCAID() {
    return refinedCaPrefix + this.number;
  }
  ...
  public static RefinedSafetyEntry getAnytimeEntry(int number, ContextTableCombination var,
      IExtendedDataModel controller) {

    RefinedSafetyEntry entry = new RefinedSafetyEntry(number, IValueCombie.TYPE_ANYTIME, IValueCombie.CONTEXT_PROVIDED,
        var, controller);
  ...
  public static RefinedSafetyEntry getTooLateEntry(int number, ContextTableCombination var,
      IExtendedDataModel controller) {
    RefinedSafetyEntry entry = new RefinedSafetyEntry(number, IValueCombie.TYPE_TOO_LATE, IValueCombie.CONTEXT_PROVIDED,
        var, controller);
  ...
  public static RefinedSafetyEntry getTooEarlyEntry(int number, ContextTableCombination var,
      IExtendedDataModel controller) {
    RefinedSafetyEntry entry = new RefinedSafetyEntry(number, IValueCombie.TYPE_TOO_EARLY,
        IValueCombie.CONTEXT_PROVIDED, var, controller);
  ...
  public static RefinedSafetyEntry getNotProvidedEntry(int number, ContextTableCombination var,
      IExtendedDataModel controller) {

    RefinedSafetyEntry entry = new RefinedSafetyEntry(number, IValueCombie.TYPE_NOT_PROVIDED,
        IValueCombie.CONTEXT_NOT_PROVIDED, var, controller);
