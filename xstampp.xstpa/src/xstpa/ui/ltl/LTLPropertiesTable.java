...
public class LTLPropertiesTable extends CommonGridView<IExtendedDataModel> {
  ...
  public DeleteGridEntryAction<IExtendedDataModel> getDeleteAction() {
    return new DeleteLTLaction(getGridWrapper(), getDataModel(), "LTL", getPREFIX());
  }

  protected void fillTable() {
    if (this.getGridWrapper() != null) {
      this.getGridWrapper().clearRows();
      List<AbstractLTLProvider> rulesList = getDataModel().getExtendedDataController().getAllScenarios(true, false,
          false);
      List<AbstractLTLProvider> scenarioList = getDataModel().getExtendedDataController().getAllScenarios(false, false,
          true);
      int causalIndex = 0;
      int basicIndex = 0;
      ...
      for (int i = 0; i < loopSize; i++) {
        ...
        if (rule != null && !isFiltered(rule.getNumber(), categoryLTL)) {
          ...
          IGridCell cell = new ScenarioEditor(this.getGridWrapper(), getPREFIX() + rule.getNumber(), showDelete, true,
              rule.getRuleId());
