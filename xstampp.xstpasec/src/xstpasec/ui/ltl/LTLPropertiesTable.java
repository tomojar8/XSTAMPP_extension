...
import xstpa.ui.ltl.DeleteLTLaction;
...
public class LTLPropertiesTable extends CommonGridView<IExtendedDataModel> {
  ...
  private class AddCSButton extends GridCellButton {
    ...
    public void onMouseDown(MouseEvent e,
      ...
      if (e.button == 1) {
        ...
        UUID newUCA = getDataModel().getExtendedDataController().addRuleEntry(
            IExtendedDataModel.ScenarioType.CUSTOM_LTL, data, null, IValueCombie.TYPE_ANYTIME, getDataModel().getLinkController());
  ...
  private class ScenarioEditor extends GridCellTextEditor {
    ...
    public void updateDataModel(String newValue) {
      ...
      getDataModel().getExtendedDataController().updateRefinedRule(getUUID(), data, null);
    }
  ...
  protected void fillTable() {
    if (this.getGridWrapper() != null) {
      ...
      List<AbstractLTLProvider> rulesList = getDataModel().getExtendedDataController().getAllScenarios(true, false, false);
      List<AbstractLTLProvider> scenarioList = getDataModel().getExtendedDataController().getAllScenarios(false, false, true);
      int causalIndex = 0;
      int basicIndex = 0;
      ...
      for (int i = 0; i < loopSize; i++) {
        ...
        if (rule != null && !isFiltered(rule.getNumber(), categoryLTL)) {
          ...
          IGridCell cell = new ScenarioEditor(this.getGridWrapper(), PREFIX + rule.getNumber(), showDelete, true,
              rule.getRuleId());
          ruleRow.addCell(0, cell);

          ruleRow.addCell(1,
              new ScenarioEditor(this.getGridWrapper(), rule.getLtlProperty(), false, !showDelete, rule.getRuleId()) {
                @Override
                public void updateDataModel(String newValue) {
                  AbstractLtlProviderData data = new AbstractLtlProviderData();
                  data.setLtl(newValue);
