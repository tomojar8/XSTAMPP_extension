...
public class WelcomeView extends ViewPart implements IEditorBase {
   ...
   * 
   * </p>
   * <i>the position is fix due to the static background image </i>
   * 
   * @author Lukas Balzer
   * 
   * @param parent
  ...
  private void addRecentProjects(Composite parent) {
    ...
    recentLabel.setFont(new Font(Display.getCurrent(), WelcomeView.TitleFont));
  ...
  public void createPartControl(final Composite parent) {
    ...
    /*
     * ----adds the interactive help panel which is displayed to the right of
     * the quickstart Buttons
     */
    ...
    /*
     * ---- adds a checkbox with which the user can set the whether or not to
     * display the welcome view on startup
     */
    ...
    checkBox.addMouseTrackListener(new MouseTrackListener() {
      ...
      public void mouseExit(MouseEvent e) {
        WelcomeView.this.getViewSite().getActionBars().getStatusLineManager().setMessage(""); //$NON-NLS-1$
      }
  ...
  private class RecentProjectLabel implements MouseListener, MouseTrackListener {
    ...
    RecentProjectLabel(Composite parent, String path, FormData data) {
      ...
      this.link = path;
      Font recentFont = new Font(Display.getCurrent(), WelcomeView.DefaultFont);
